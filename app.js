...
const authenticationToken = (request,response,next)=>{
  ...
  if (jwtToken!==undefined){
    jwt.verify(jwtToken, "secret_key", async(request,response)=>{
     if(error){
        response.status(401);
         response.send("Invalid IWT Token");
     } else{
...
/**get following id from user ids */
const getFollowerIdsQuery=`select following_user_id from follower
where follower_user_id=${getUserId.user.id};`;
const getFollowersIds=await database.all(getFollowerIdsQuery);
...
app.get("/user/followers/", authenticationToken, async (request,response)=>{
    let {username}=request;
    const getUserIdQuery=`select user_id from user where username='${username}';`;
    const getUserId= await database.all(getFollowerIdsQuery);
...
app.get("/tweets/:tweetId/", authenticationToken, async(request,response)=>{
    ...
    //console.log(followingTweetIds.includes(parsInt(tweetId)));
    ...
    //console.log(likes_count)
     const reply_count_query=`select count(user_id ) as replies from reply where tweet_id=${tweetId};`;
        const reply_count= await database.get(likes_count_query);
...
//API 7 
const convertLikedNameDBObjectToResponseObject=(dbObject)=>{
    return {
        likes: dbObject,
    };
};
...
async(request,response)=>{
    ...
    //console.log(tweetId)
    let{username}=request;
    const getUserIdQuery=`select user_id from where user_name='${username}';`;
    const getUserId= await database.get(getUserIdQuery);
    //console.log(getUserId);
    ...
    if(getTweetIds.includes(parseInt(tweetId))){
        const getLikedUsersNameQuery=`select user.username as likes from user inner join like on user.user_id=like.user_id
         ...
         /* console.log(convertLikedUserNameDBObjectToResponseObject(getLikedUserNames));*/
         response.send(convertLikedUserNameDBObjectToResponseObject(getLikedUserNames))
...
app.get(
    ...
    async(request,response)=>{
        ...
        let { username }=request;
        const getUserIdQuery=`select following_user_id from follower where follower_user_id=${getUserId.user_id};`;
        const getFollowingIdsArray= await database.all(getFollowingIdsQuery);
        //console.log(getFollowingIdsArray);
        const getFollowingIds=getFollowingIdsArray.map((eachFollower)=>{
            return eachFollower.follower_user_id;
        ...
        if(getTweetIds.includes(parseInt(tweetId))){
            ...
            const getUsernameReplyTweetQuery=`select user.name, reply.reply from user innner join reply on user.user_id=reply.reply_id
            where reply.tweet_id=${tweetId};`;
            const getUsernameReplyTweets= await database.all(
                getUsernameReplyTweetQuery
            );
            //console.log(getUsernameReplyTweets);
            /* console.log(convertUserNameReplyDBObject(getUserNameReplyTweets))*/

            response.send(convertUserNameReplyDBObjectToResponseObject(getUserNameReplyTweets));
        } else{
            response.status(401);
...
//api 9
 app.get("/user/tweets/", authenticationToken, async(request,response)=>{
     let {username}=request;
     const getUserIdQuery=`select user_id from your where username='${username}';`;
     const getUserId=await database.all(getTweetIdsQuery);
 ...
 app.post("/user/tweets", authenticationToken, async(request,response)=>{
     ...
     //console.log(tweet)
     //const currentDate=format(newDate(), "yyyy-MM-dd- HH-mm-ss");
 ...
 app.delete("/tweets/:tweetId",authenticationToken, async(request,response)=>{
   const{ tweetId }=request.params;
   let {username}=request;
   const getUserIdQuery=`select user_id from where username='${username}';`;
   const getUserId=await database.get(getUserIdQuery);
